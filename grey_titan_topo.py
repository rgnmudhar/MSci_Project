...
diag.add_field('mixed_layer', 'land', time_avg=True) #!! new line
...
#     },
    ...
    'spectral_init_cond_nml': { #namelist additional to the Mars script
        'topog_file_name' : 'titan_land_t42.nc', #!!! new line - name of land input file, which will also contain topography
        'topography_option' : 'input', #!!! new line - tells model to get topography from input file
    ...
    for conv in conv_schemes:
        for depth_val in depths:
            for per_value in pers:
                exp = Experiment('grey_titan_topo', codebase=cb) #name of folder in which .nc files are output
                exp.clear_rundir()
                ...
                exp.inputfiles = [os.path.join(GFDL_BASE,'input/land_masks/titan_land_t21.nc')] #!!! new line - location of topography file
